...
async fn simple_concat() {
    ...
    let original_names: Vec<String> =
        decode_names(&mut Archive::new(archive_bytes.as_slice())).await;
    ...
    let actual = decode_names(&mut Archive::new(archive_bytes.as_slice())).await;
    ...
    let mut ar = Archive::new(archive_bytes.as_slice());
...
async fn header_impls() {
    let mut ar = Archive::new(tar!("simple.tar"));
...
async fn header_impls_missing_last_header() {
    let mut ar = Archive::new(tar!("simple_missing_last_header.tar"));
    let hn = Header::new_old();
    let hnb = hn.as_bytes();
    let mut entries = t!(ar.entries());
    while let Some(file) = entries.next().await {
...
async fn large_filename() {
    ...
    let mut ar = Archive::new(rd.as_slice());
    let mut entries = t!(ar.entries());
...
async fn reading_entries_with_seek() {
    let rdr = tar!("reading_files.tar");
    let mut ar = Archive::new(Cursor::new(rdr));
...
impl<T: Read> Read for LoggingReader<T> {
    fn poll_read(
        ...
        match this.inner.poll_read(cx, buf) {
            Poll::Ready(Ok(_)) => {
                *this.read_bytes += buf.filled().len() as u64;
                Poll::Ready(Ok(()))
            }
            res @ _ => res,
        }
...
async fn skipping_entries_with_seek() {
    ...
    let mut entries = t!(ar_reader.entries());
    let mut files: Vec<_> = vec![];
    while let Some(entry) = entries.next().await {
        files.push(entry.unwrap().path().unwrap().to_path_buf());
    }
    ...
    let mut files_seekable = vec![];
    let mut entries = t!(ar_seekable_reader.entries_with_seek());
    while let Some(entry) = entries.next().await {
        files_seekable.push(entry.unwrap().path().unwrap().to_path_buf());
    }

    assert_eq!(files, files_seekable);
...
async fn handling_incorrect_file_size() {
    ...
    assert!(v.iter().any(|fr| fr.is_err()));
}
...
async fn zero_file_times() {
    ...
    let path = td.path().join("tmpfile");
    t!(File::create(&path).await);
    t!(ar.append_path_with_name(&path, "a").await);
    ...
    let mtime = FileTime::from_last_modification_time(&meta);
    let atime = FileTime::from_last_access_time(&meta);
    assert_ne!(mtime.unix_seconds(), 0);
    assert_ne!(atime.unix_seconds(), 0);
}
...
async fn backslash_treated_well() {
    ...
    let b = t!(ar.into_inner().await);
    let mut ar = Archive::new(b.as_slice());
...
#[tokio::test]
async fn links() {
    let mut ar = Archive::new(tar!("link.tar"));
    let mut entries = t!(ar.entries());
...
async fn pax_linkpath() {
    ...
    let entries = async {
        ...
        while let Some(entry) = e.next().await {
            v.push(entry);
        }
    ...
    .await;
    let mut links = entries.iter().skip(3).take(2);
...
async fn name_with_slash_doesnt_fool_long_link_and_bsd_compat() {
    ...
    .await;
    assert!(entries.iter().all(|fr| fr.is_ok()));
...
async fn ownership_preserving() {
    ...
    if unsafe { libc::getuid() } == 0 {
        ...
        let meta = fs::metadata(td.path().join("iamuid580800000"))
            .await
            .unwrap();
        assert_eq!(meta.uid(), 580800000);
        assert_eq!(meta.gid(), 580800000);
        let meta = fs::metadata(td.path().join("iamuid580800001"))
            .await
            .unwrap();
        assert_eq!(meta.uid(), 580800001);
        assert_eq!(meta.gid(), 580800000);
        let meta = fs::metadata(td.path().join("iamuid580800002"))
            .await
            .unwrap();
