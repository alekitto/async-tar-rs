...
async fn absolute_symlink() {
    let mut ar = async_tar_rs::Builder::new(Vec::new());

    let mut header = async_tar_rs::Header::new_gnu();
    header.set_size(0);
    header.set_entry_type(async_tar_rs::EntryType::Symlink);
    ...
    let mut ar = async_tar_rs::Archive::new(&bytes[..]);
...
async fn absolute_hardlink() {
    ...
    let mut header = async_tar_rs::Header::new_gnu();
    header.set_size(0);
    header.set_entry_type(async_tar_rs::EntryType::Regular);
    t!(header.set_path("foo"));
    header.set_cksum();
    t!(ar.append(&header, &[][..]).await);
    ...
    let mut header = async_tar_rs::Header::new_gnu();
    header.set_size(0);
    header.set_entry_type(async_tar_rs::EntryType::Link);
...
async fn directory_maintains_permissions() {
    ...
    let mut header = async_tar_rs::Header::new_gnu();
    header.set_size(0);
    header.set_entry_type(async_tar_rs::EntryType::Directory);
    t!(header.set_path("foo"));
...
async fn set_entry_mask() {
    ...
    let f = t!(File::open(foo_path).await);
    let md = t!(f.metadata().await);
    assert!(md.is_file());
