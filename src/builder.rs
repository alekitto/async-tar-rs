...
use async_std::{
    fs::{self, File},
    io::{self, empty, Read, ReadExt, Seek, Write, WriteExt},
};
#[cfg(feature = "async-std")]
...
use tokio::{
    fs::{self, File},
    io::{
        self, empty, AsyncRead as Read, AsyncReadExt, AsyncSeek as Seek, AsyncSeekExt as SeekExt,
        AsyncWrite as Write, AsyncWriteExt,
    },
};
...
impl<W: Write + Unpin + Send + Sync> Builder<W> {
    ...
    /// let mut entry = ar.append_writer(&mut header, "hi.txt")?;
    /// entry.write_all(b"Hello, ").await?;
    /// entry.write_all(b"world!\n").await?;
    /// entry.finish()?;
    /// # Ok(()) }
    /// ```
    pub async fn append_writer<'a, P: AsRef<Path>>(
        &'a mut self,
        header: &'a mut Header,
        path: P,
    ...
    where
        W: Seek,
    {
        EntryWriter::start(self.get_mut(), header, path.as_ref()).await
    }
    ...
    async fn _append_link(
        &mut self,
        header: &mut Header,
        path: &Path,
        target: &Path,
    ) -> io::Result<()> {
        prepare_header_path(self.get_mut(), header, path).await?;
        prepare_header_link(self.get_mut(), header, target).await?;
        header.set_cksum();
        self.append(&header, empty()).await
    }
    ...
    pub async fn append_path<P: AsRef<Path>>(&mut self, path: P) -> io::Result<()> {
        ...
        append_path_with_name(self.get_mut(), path.as_ref(), None, options).await?;
    ...
    pub async fn append_path_with_name<P: AsRef<Path>, N: AsRef<Path>>(
        ...
        let options = self.options;
        append_path_with_name(self.get_mut(), path.as_ref(), Some(name.as_ref()), options).await?;
    ...
    pub async fn append_file<P: AsRef<Path>>(
        ...
        let options = self.options;
        append_file(self.get_mut(), path.as_ref(), file, options).await?;
        Ok(())
    }
    ...
    {
        let options = self.options;
        append_dir(self.get_mut(), path.as_ref(), src_path.as_ref(), options).await?;
        Ok(())
    }
    ...
    /// use async_tar_rs::{Archive, Builder};
    ///
    /// let tmpdir = tempfile::tempdir().unwrap();
    /// let path = tmpdir.path();
    /// fs::write(path.join("a.txt"), b"hello").await?;
    /// fs::write(path.join("b.txt"), b"world").await?;
    ///
    ...
    /// let archive = ar.into_inner().await.unwrap();
    ...
    {
        let options = self.options;
        append_dir_all(self.get_mut(), path.as_ref(), src_path.as_ref(), options).await?;
        Ok(())
    }
...
trait SeekWrite: Write + Seek + Send + Unpin {
    fn as_write(&mut self) -> &mut (dyn Write + Send + Unpin);
}

impl<T: Write + Seek + Send + Unpin> SeekWrite for T {
    fn as_write(&mut self) -> &mut (dyn Write + Send + Unpin) {
        self
    }
}
...
impl EntryWriter<'_> {
    async fn start<'a>(
        ...
        prepare_header_path(obj.as_write(), header, path).await?;

        // Reserve space for header, will be overwritten once data is written.
        obj.write_all([0u8; 512].as_ref()).await?;
    ...
    pub async fn finish(self) -> io::Result<()> {
        ...
        this.do_finish().await
    }

    async fn do_finish(&mut self) -> io::Result<()> {
        // Pad with zeros if necessary.
        let buf = [0u8; 512];
        let remaining = u64::wrapping_sub(512, self.written) % 512;
        self.obj.write_all(&buf[..remaining as usize]).await?;
        let written = (self.written + remaining) as i64;

        // Seek back to the header position.
        self.obj.seek(io::SeekFrom::Current(-written - 512)).await?;

        self.header.set_size(self.written);
        self.header.set_cksum();
        self.obj.write_all(self.header.as_bytes()).await?;

        // Seek forward to restore the position.
        self.obj.seek(io::SeekFrom::Current(written)).await?;
...
impl Write for EntryWriter<'_> {
    fn poll_write(
        ...
        let len = ready!(this.obj.poll_write(cx, buf))?;
        *this.written += len as u64;
        Poll::Ready(Ok(len))
    }
...
async fn append(
    ...
    pad_zeroes(dst, len).await?;

    Ok(())
}

async fn pad_zeroes(dst: &mut (dyn Write + Unpin + Send), len: u64) -> io::Result<()> {
    let buf = [0; 512];
...
async fn append_path_with_name(
    ...
    if stat.is_file() {
        append_file(dst, ar_name, &mut File::open(path).await?, options).await
    } else if stat.is_dir() {
        append_fs(dst, ar_name, &stat, options.mode, None).await
    } else if stat.file_type().is_symlink() {
        let link_name = fs::read_link(path).await?;
        append_fs(dst, ar_name, &stat, options.mode, Some(&link_name)).await
    } else {
        #[cfg(unix)]
        {
            append_special(dst, path, &stat, options.mode).await
        }
        #[cfg(not(unix))]
        {
...
async fn append_file(
    ...
    let stat = file.metadata().await?.into();
    let mut header = Header::new_gnu();

    prepare_header_path(dst, &mut header, path).await?;
    header.set_metadata_in_mode(&stat, options.mode);
    let sparse_entries = if options.sparse {
        prepare_header_sparse(file, &stat, &mut header).await?
    } else {
        None
    };
    header.set_cksum();
    ...
    if let Some(sparse_entries) = sparse_entries {
        append_extended_sparse_headers(dst, &sparse_entries).await?;
        for entry in sparse_entries.entries {
            file.seek(io::SeekFrom::Start(entry.offset)).await?;
            io::copy(&mut file.take(entry.num_bytes), dst).await?;
        }
        pad_zeroes(dst, sparse_entries.on_disk_size).await?;
    } else {
        let len = io::copy(file, dst).await?;
        pad_zeroes(dst, len).await?;
    }
...
async fn append_dir(
    ...
    append_fs(dst, path, &stat, options.mode, None).await?;
    Ok(())
}
...
async fn prepare_header_sparse(
    file: &mut fs::File,
    stat: &std::fs::Metadata,
    header: &mut Header,
) -> io::Result<Option<SparseEntries>> {
    let entries = match find_sparse_entries(file, stat).await? {
...
async fn append_extended_sparse_headers(
    dst: &mut (dyn Write + Send + Unpin),
    ...
    while it.peek().is_some() {
        ...
        dst.write_all(ext_header.as_bytes()).await?;
    }
...
async fn append_dir_all(
    ...
    while let Some((src, is_dir, is_symlink)) = stack.pop() {
        ...
        if is_dir || (is_symlink && options.follow && src.is_dir()) {
            ...
            if dest != Path::new("") {
                append_dir(dst, &dest, &src, options).await?;
            }
        } else if !options.follow && is_symlink {
            let stat = fs::symlink_metadata(&src).await?;
            let link_name = std::fs::read_link(&src)?;
            append_fs(dst, &dest, &stat, options.mode, Some(&link_name)).await?;
        } else {
            #[cfg(unix)]
            {
                let stat = std::fs::metadata(&src)?;
                if !stat.is_file() {
                    append_special(dst, &dest, &stat, options.mode).await?;
                    continue;
                }
            }

            append_file(dst, &dest, &mut File::open(src).await?, options).await?;
        }
    }
...
async fn find_sparse_entries(
    ...
    find_sparse_entries_seek(file, stat).await
}

/// Implementation of `find_sparse_entries` using `SEEK_HOLE` and `SEEK_DATA`.
#[cfg(any(target_os = "android", target_os = "freebsd", target_os = "linux"))]
async fn find_sparse_entries_seek(
    file: &mut File,
    stat: &std::fs::Metadata,
) -> io::Result<Option<SparseEntries>> {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_find_sparse_entries() {
        ...
        for &(description, map) in cases {
            ...
            for e in map {
                file.seek(io::SeekFrom::Start(e.offset)).await.unwrap();
            ...
            let stat = file.metadata().await.unwrap();
            let reported = find_sparse_entries(&mut file, &stat).await.unwrap();
    ...
    assert_impl_all!(Builder<File>: Send, Sync);
}
